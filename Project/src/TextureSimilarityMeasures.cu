...
__device__ uint8_t getPixel(ColorRG *image, int x, int y, size_t pitch, int width, int height) {
    if (x < 0 || x >= width || y < 0 || y >= height) {
        return 0;
    }
    ColorRG pixel = image[y * pitch / sizeof(ColorRG) + x];
...
__global__ void TextureSimilarityKernel(ColorRG *d_img1, size_t pitch1, ColorRG *d_img2, size_t pitch2, double *d_similarities, size_t pitch, int width, int height) {
    ...
    if (x < width && y < height) {
        uint8_t centerPixel1 = (d_img1[y * pitch1 / sizeof(ColorRG) + x].r + d_img1[y * pitch1 / sizeof(ColorRG) + x].g) / 2;
        uint8_t centerPixel2 = (d_img2[y * pitch2 / sizeof(ColorRG) + x].r + d_img2[y * pitch2 / sizeof(ColorRG) + x].g) / 2;
