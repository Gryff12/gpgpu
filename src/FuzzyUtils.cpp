...
void FuzzyUtils::LBP(IplImage *InputImage, IplImage *LBPimage) {
    ...
    *(CarreExp + 8) = 128.0;
    ...
    if (x == 0 && y == 0) {
        ...
        *valLBP = *valLBP + ((*(BinaryValue + 1)) * (*(CarreExp + 1)) + (*(BinaryValue + 2)) * (*(CarreExp + 2)) +
                             (*(BinaryValue + 3)) * (*(CarreExp + 3))) / 255.0;
    ...
    if (x == 0 && y == InputImage->width) {
        ...
        *valLBP = *valLBP + ((*(BinaryValue)) * (*(CarreExp)) + (*(BinaryValue + 2)) * (*(CarreExp + 2)) +
                             (*(BinaryValue + 3)) * (*(CarreExp + 3))) / 255.0;
        p.PutGrayPixel(LBPimage, x, y, *valLBP);
    }
    ...
    if (x == InputImage->height && y == 0) {
        ...
        *valLBP = *valLBP + ((*(BinaryValue)) * (*(CarreExp)) + (*(BinaryValue + 1)) * (*(CarreExp + 1)) +
                             (*(BinaryValue + 3)) * (*(CarreExp + 3))) / 255.0;
        p.PutGrayPixel(LBPimage, x, y, *valLBP);
    }
    ...
    if (x == InputImage->height && y == InputImage->width) {
        ...
        *valLBP = *valLBP + ((*(BinaryValue)) * (*(CarreExp)) + (*(BinaryValue + 1)) * (*(CarreExp + 1)) +
                             (*(BinaryValue + 2)) * (*(CarreExp + 2))) / 255.0;
        p.PutGrayPixel(LBPimage, x, y, *valLBP);
    }
    ...
    if (x == 0 && (y != 0 && y != InputImage->width)) {
        for (int y = 1; y < InputImage->width - 1; y++) {
            ...
            *valLBP = 0;
            *valLBP = *valLBP + ((*(BinaryValue)) * (*(CarreExp)) + (*(BinaryValue + 1)) * (*(CarreExp + 1)) +
                                 (*(BinaryValue + 2)) * (*(CarreExp + 2)) + (*(BinaryValue + 3)) * (*(CarreExp + 3)) +
                                 (*(BinaryValue + 5)) * (*(CarreExp + 5))) / 255.0;
            p.PutGrayPixel(LBPimage, x, y, *valLBP);
        }
    }
    ...
    if (x == InputImage->height && (y != 0 && y != InputImage->width)) {
        for (int y = 1; y < InputImage->width - 1; y++) {
            ...
            *valLBP = 0;
            *valLBP = *valLBP + ((*(BinaryValue)) * (*(CarreExp)) + (*(BinaryValue + 2)) * (*(CarreExp + 2)) +
                                 (*(BinaryValue + 3)) * (*(CarreExp + 3)) + (*(BinaryValue + 4)) * (*(CarreExp + 4)) +
                                 (*(BinaryValue + 5)) * (*(CarreExp + 5))) / 255.0;
            p.PutGrayPixel(LBPimage, x, y, *valLBP);
        }
    }
    ...
    if ((x != 0 && x != InputImage->height) && y == 0) {
        for (int x = 1; x < InputImage->height - 1; x++) {
            ...
            *valLBP = 0;
            *valLBP = *valLBP + ((*(BinaryValue)) * (*(CarreExp + 5)) + (*(BinaryValue + 1)) * (*(CarreExp + 6)) +
                                 (*(BinaryValue + 3)) * (*(CarreExp + 3)) + (*(BinaryValue + 4)) * (*(CarreExp)) +
                                 (*(BinaryValue + 5)) * (*(CarreExp + 1))) / 255.0;
            p.PutGrayPixel(LBPimage, x, y, *valLBP);
        }
    }
...
void FuzzyUtils::SimilarityDegreesImage(IplImage *CurrentImage, IplImage *BGImage, IplImage *DeltaImage, int n,
                                        int color_space) {
    PixelUtils p;
    int i, j;
    if (n == 1) {
    ...
    if (n != 1) {
        IplImage *ConvertedCurrentImage = cvCreateImage(cvSize(CurrentImage->width, CurrentImage->height),
                                                        IPL_DEPTH_32F, 3);
...
void
FuzzyUtils::AdaptativeSelectiveBackgroundModelUpdate(IplImage *CurrentImage, IplImage *BGImage, IplImage *OutputImage,
                                                     IplImage *Integral, float seuil, float alpha) {
    PixelUtils p;
    float beta = 0.0;
    ...
    for (int i = 0; i < CurrentImage->width; i++) {
        for (int j = 0; j < CurrentImage->height; j++) {
            ...
            beta = 1 - ((*IntegralImagePixel) - ((*Minimum / (*Minimum - *Maximum)) * (*IntegralImagePixel) -
                                                 (*Minimum * (*Maximum) / (*Minimum - *Maximum))));
            for (int k = 0; k < 3; k++)
                *(OutputImagePixel + k) = beta * (*(BGImagePixel + k)) + (1 - beta) *
                                                                         (alpha * (*(CurentImagePixel + k)) +
                                                                          (1 - alpha) * (*(BGImagePixel + k)));
