...
void FuzzyChoquetIntegral::process(const cv::Mat &img_input, cv::Mat &img_output, cv::Mat &img_bgmodel) {
    ...
    if (frameNumber <= framesToLearn) {
        ...
        IplImage *lbp_background_f1 = cvCreateImage(cvSize(background_f1->width, background_f1->height), IPL_DEPTH_32F,
                                                    1);
        ...
        fu.AdaptativeSelectiveBackgroundModelUpdate(input_f3, background_f3, updated_background_f3, integral_choquet_f1,
                                                    threshold, alphaUpdate);
